...
public class InsertionSort {
	...
	public static void insertionSort(int[] arr) {
		...
		// Loop through the array starting from the second element (index 1) as we
		// assumed that the first element is at the right place
		for (int i = 1; i < arr.length; i++) {
			// Store the current element
			int current = arr[i];

			// Initialize j with i - 1
			int j = i - 1; // Initialize j to point to the element before current
